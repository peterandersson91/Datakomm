...
/*static void *BroadcastFunction(void *sock)
 {
 int broadcast*
 calloc()
...
int main(int argc, char *argv[]) {
    ...
    socklen_t size;
    char messageString[50] = "Servern hör dig!";
    char broadcastMsg[50] = "A new client connected.\n";
    char blockAdress[50] = "127.0.0.2";
    memset(connectedClients, 0, sizeof(int) * MAXCLIENTS);
        ...
        for(i = 0; i < FD_SETSIZE; ++i)
            if(FD_ISSET(i, &readFdSet)) {
                if(i == sock) {
                    ...
                    if(strcmp(inet_ntoa(clientName.sin_addr), blockAdress) == 0)
                    {
                        printf("Connection refused from %s\n", inet_ntoa(clientName.sin_addr));
                        write(clientSocket, "Connection refused by Peter", 28);
                        close(clientSocket);
                    }
                    else
                    {
                        ...
                        write(clientSocket, messageString, strlen(messageString) + 1);
                        connectedClients[NumOfClients] = clientSocket;
                        broadcast = NumOfClients;
                        while(broadcast >= 0)
                        {
                            //  printf("%s\n", clientName.sin_port);
                            //  if()
                            if(broadcast != clientSocket)
                                write(connectedClients[broadcast], broadcastMsg, strlen(broadcastMsg) + 1);
                            usleep(10);
                            broadcast--;
                        }
                        NumOfClients++;
